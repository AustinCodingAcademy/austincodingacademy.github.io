...
classics.forEach(key => {
  ...
  packageJson.scripts['deploy-classics'] = `${packageJson.scripts['deploy-classics']} && npm run clear-cache && npm run favicon-${key} && npm run build-${key} && npm run sitemap-${key} && npm run deploy-${key}`;
  packageJson.scripts['deploy-classics-preview'] = `${packageJson.scripts['deploy-classics-preview']} && npm run clear-cache && npm run favicon-${key} && npm run build-${key} && npm run sitemap-${key} && npm run deploy-${key}-preview`;
});

remotes.forEach(key => {
  packageJson.scripts['deploy-remotes'] = `${packageJson.scripts['deploy-remotes']} && npm run clear-cache && npm run build-${key} && npm run sitemap-${key} && npm run deploy-${key}`;
});
