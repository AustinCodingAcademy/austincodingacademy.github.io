...
function buildScript(key, deployScript) {
  ...
  packageJson.scripts[deployScript] += ` && KEY=${key} yarn sass && yarn sass-include && KEY=${key} yarn build && rsvg-convert -h 100 _site/logo-white.svg > logo-white.png && KEY=${key} yarn favicon && KEY=${key} yarn build && KEY=${key} yarn sitemap && KEY=${key} yarn cname && KEY=${key} yarn repo && yarn clear-cache`;
}
